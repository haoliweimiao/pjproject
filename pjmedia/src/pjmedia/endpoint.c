 ...
 */
PJ_DEF(pj_status_t)
pjmedia_endpt_create2(pj_pool_factory *pf,
					  pj_ioqueue_t *ioqueue,
					  unsigned worker_cnt,
 ...
 */
PJ_DEF(pjmedia_codec_mgr *)
pjmedia_endpt_get_codec_mgr(pjmedia_endpt *endpt)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_endpt_destroy2(pjmedia_endpt *endpt)
{
	exit_cb *ecb;
...
PJ_DEF(pj_status_t)
pjmedia_endpt_set_flag(pjmedia_endpt *endpt,
...
PJ_DEF(pj_status_t)
pjmedia_endpt_get_flag(pjmedia_endpt *endpt,
					   pjmedia_endpt_flag flag,
					   void *value)
{
	PJ_ASSERT_RETURN(endpt, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_ioqueue_t *)
pjmedia_endpt_get_ioqueue(pjmedia_endpt *endpt)
{
 ...
 */
PJ_DEF(pj_thread_t *)
pjmedia_endpt_get_thread(pjmedia_endpt *endpt,
						 unsigned index)
{
	PJ_ASSERT_RETURN(endpt, NULL);
 ...
 */
PJ_DEF(pj_pool_t *)
pjmedia_endpt_create_pool(pjmedia_endpt *endpt,
						  const char *name,
						  pj_size_t initial,
						  pj_size_t increment)
{
							   ...
							   pjmedia_sdp_media **p_m)
{
	...
	status = init_sdp_media(m, pool, &STR_AUDIO, si, options ? options->dir : param.dir);
	if (status != PJ_SUCCESS)
		return status;
							   ...
							   pjmedia_sdp_media **p_m)
{
	...
	pjmedia_endpt_create_sdp_param_default(&param);
	m = PJ_POOL_ZALLOC_T(pool, pjmedia_sdp_media);
	status = init_sdp_media(m, pool, &STR_VIDEO, si, options ? options->dir : param.dir);
	if (status != PJ_SUCCESS)
		return status;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_endpt_create_base_sdp(pjmedia_endpt *endpt,
							  pj_pool_t *pool,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_endpt_create_sdp(pjmedia_endpt *endpt,
						 pj_pool_t *pool,
						 unsigned stream_cnt,
						 const pjmedia_sock_info sock_info[],
						 pjmedia_sdp_session **p_sdp)
...
PJ_DEF(pj_status_t)
pjmedia_endpt_dump(pjmedia_endpt *endpt)
{
...
PJ_DEF(pj_status_t)
pjmedia_endpt_atexit(pjmedia_endpt *endpt,
					 pjmedia_endpt_exit_callback func)
{
	exit_cb *new_cb;
